...
| # | 状態 | タスク | 層 | 対象ファイル | 依存 |
|---|------|--------|----|-------------|------|
| 1 | [ ] | シンボリックリンク越しの読み取りを実地検証する | 検証（変更なし） | scratchpad のサンドボックス | - |
...
**先行実装での実測値（先行実装B の `.agents/plans/plan-agents-dir-symlink.md` タスク1、2026-08-28 実測 / macOS 25.6.0）** — 同じマシン・同日の測定なので、再測定して食い違わないかの照合に使う:

| 対象 | 走査中に遭遇したリンク | 引数で明示的に渡したリンク |
|---|---|---|
| Read ツール | — | **辿る** |
| Glob / Grep ツール | 測定不能（このビルドに存在しない） | 測定不能 |
| `find` | 辿らない（`-L` が要る） | **辿らない**。ただし末尾に `/` を付けると辿る |
| `rg` | 辿らない（`--follow` が要る） | **辿る** |
| シェル glob | 辿る | 辿る |
